...
## Использование Gifsicle для оптимизации потребления памяти

Приложение поддерживает обработку GIF-файлов с использованием внешней утилиты [Gifsicle](https://www.lcdf.org/gifsicle/), которая значительно снижает потребление памяти при сжатии GIF-анимаций.

### Преимущества использования Gifsicle

- **Низкое потребление памяти**: Gifsicle требует в 5-10 раз меньше памяти для обработки GIF-файлов по сравнению с Java-реализацией
- **Высокая скорость сжатия**: Специализированная утилита работает быстрее, особенно для больших анимаций
- **Качественная оптимизация**: Поддерживает различные уровни оптимизации, включая сжатие с потерями
- **Отказоустойчивость**: При недоступности Gifsicle приложение автоматически использует встроенную Java-реализацию

### Настройка Gifsicle

1. **Установка Gifsicle**:
   - Windows: `winget install gifsicle` или скачать с [официального сайта](https://www.lcdf.org/gifsicle/)
   - Linux: `apt-get install gifsicle` или `yum install gifsicle`
   - macOS: `brew install gifsicle`

2. **Конфигурация в application.yml**:
   ```yaml
   gifsicle:
     # Путь к исполняемому файлу, по умолчанию предполагается, что он доступен в PATH
     path: gifsicle
     # Включить Gifsicle как основной процессор
     enabled: true
     # Уровень оптимизации (1-3)
     optimization-level: 3
     # Использовать ли сжатие с потерями
     use-lossy: true
     # Уровень потери качества (0-100)
     lossy-level: 30
   ```

### Сравнение потребления ресурсов

| Метод обработки | Потребление памяти (для GIF 0.5MB) | Скорость обработки |
|-----------------|-----------------------------------|-------------------|
| Java (встроенный) | ~120MB                           | Базовая           |
| Gifsicle        | ~12MB                            | В 2-3 раза быстрее |
